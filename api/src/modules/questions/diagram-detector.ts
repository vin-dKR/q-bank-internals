...
/**
 * The position of one numbered question's first line on the page, in natural page pixels: `yTop` (top
 * of the line) and `xLeft` (left edge of the question number). Every numbered question is reported,
 * with or without a diagram. This is the layout anchor the matcher uses to attach a figure to the
 * question directly above it *in the same column* — far more reliable than the model's own per-figure
 * question number, which it must infer in one combined reasoning step and routinely gets wrong on
 * two-column papers. `xLeft` is what lets the matcher keep left-column figures off right-column
 * questions.
 */
export type QuestionTop = { qNo: number; xLeft: number; yTop: number };

/** Detection results paired with the token spend the model reported producing them. */
export type DiagramDetectionResult = {
